...
// Move a node from list q to list p
void moveNode(struct node** p, struct node** q) {
    if (*q == NULL) {  // Check if the source list (q) is empty
        printf("Source list is empty, cannot move node.\n");
        return;
    }
    struct node* temp = *q;
    *q = temp->next;   // Move head of q to the next node
    temp->next = *p;   // Link the node to destination list p
    *p = temp;         // Update head of p
}
...
int main() {
    ...
    do {
        ...
        switch (choice) {
            ...
            case 11:
                printf("Moving node from q to sourceList.\n");
                moveNode(&sourceList, &q);
