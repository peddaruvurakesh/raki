...
// Move all nodes from list q to list p
void moveAllNodes(struct node** p, struct node** q) {
    if (*q == NULL) {  // Check if the source list (q) is empty
        printf("Source list (q) is empty, nothing to move.\n");
        return;
    }

    // Find the last node of sourceList (p)
    struct node* temp = *p;
    if (temp == NULL) {
        // If sourceList (p) is empty, just point it to q
        *p = *q;
    } else {
        while (temp->next != NULL) {
            temp = temp->next;
        }
        // Attach the entire q list at the end of p
        temp->next = *q;
    }
    
    *q = NULL;  // Now q is empty after moving all its nodes
}

// Main function
int main() {
    struct node* head = NULL;      // Primary list
    struct node* sourceList = NULL;  // Source list for moving nodes
    struct node* q = NULL;         // Another list q
    ...
    do {
        ...
        printf("11. Move Nodes from q to sourceList\n");
        printf("12. Exit\n");
        printf("13. Add at Beginning of q\n");
        ...
        switch (choice) {
            ...
            case 11:
                printf("Moving all nodes from q to sourceList.\n");
                moveAllNodes(&sourceList, &q);
                printf("Source List after move: ");
                PrintList(sourceList);
                printf("q List after move: ");
                PrintList(q);
                break;
            ...
            case 13:
                printf("Enter data to add at beginning of q: ");
                scanf("%d", &data);
                AddBegin(&q, data);
                break;
