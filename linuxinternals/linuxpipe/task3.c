...
void persistent_pipeline(const char *cmd1, char *const argv1[], const char *cmd2, char *const argv2[]) {
    int pipefd[2], status;
  ...
  // close(pipefd[1]); // Close unused write end
    ...
    waitpid(child1, &status, 0); // Wait for child1 (echo foo)
    waitpid(child2, &status, 0); // Wait for child2 (wc -c)
}

int main() {
    // Example: echo foo | wc -c
    char *cmd1 = "echo";
    ...
    persistent_pipeline(cmd1, argv1, cmd2, argv2);
